...
对应 `memory_order_relaxed`，其对于内存序的限制最小，这种方式只能保证当前的数据访问是原子操作，但是对内存访问顺序没有任何约束，也就是说对不同的原子变量的读写可能会被重新排序，不同的线程可能会看到不同的执行顺序
...
与其它内存序相比，relaxed 内存序具有最少的同步开销，一般用于多线程计数器，`std::shared_ptr` 的引用计数递增就是利用这个实现的，但引用计数的递减使用 acq-rel 模型
